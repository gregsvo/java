...
public class ConsoleIO {
    ...
    public int getMenuChoice(String menu, String error, int[] options) {
        ...
        int userChoice = this.getUserInputIntMinMax(menu, options[0]-1, options[options.length]);
        while (true) {
            for (int i : options) {
                if (i==userChoice) {
                    return userChoice;
            ...
            this.print(error);
            userChoice = this.getUserInputIntMinMax(menu, options[0]-1, options[options.length]);
        }
