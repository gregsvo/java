...
/**
 * @author some guy, no idea who wrote it.
 */
...
//BEGIN: getUserInputString    
    ...
    public String getUserInputString(String question, String acceptableAnswers[], String eMsg) {
        ...
        while (notValid) {
            try {
                ...
                if (acceptableAnswers[0].equals("") || acceptableAnswers == null) {
                    notValid = false;
    ...
    public int getMenuChoice(String menu, String error, int[] options) {
        ...
        int userChoice = this.getUserInputIntMinMax(menu, options[0] - 1, options[options.length - 1] + 1);
        while (true) {
            for (int i : options) {
                if (i == userChoice) {
                    return userChoice;
            ...
            this.print(error);
            userChoice = this.getUserInputIntMinMax(menu, options[0] - 1, options[options.length - 1] + 1);
        }
...
//BEGIN: getUserInputFloat
    //asks <question>, validates from <min> to <max> with option for the comparisons to be <equalTo>
public boolean isStringANumber(String stringToParse) {
        ...
        while (true) {
            try {
                ...
                userInt = Integer.parseInt(stringToParse);
    ...
    public double parseStringToDouble(String stringToParse, String prompt) {
        ...
        while (true) {
            try {
                ...
                userDbl = Double.parseDouble(stringToParse);

                return userDbl;

            } catch (Exception ex) {
                print(eMsg);
                stringToParse = getUserInputString(prompt);
            }
        }
    }//end getUserInputDouble()  
