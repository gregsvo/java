$(document).ready(function () {
    ...
    $("#editOrderModal").on("show.bs.modal", function (e) {
        ...
        $.ajax({
            ...
            modal.find("#edit-id").val(order.order_id);
            modal.find("#edit-customer-name").val(order.customer_name);
    ...
    $(document).on('click', '#edit-order-button', function (e) {
        ...
        $.ajax({
            ...
            data: JSON.stringify({
                id: orderId,
                customerNam: $("#edit-customer-name").val(),
    ...
    function buildOrderRow(data) {
        ...
        return "<tr id='newOrderRow-" + data.order_id + "'>  \n\
                <td> <a data-order-id='" + data.order_id + "'data-toggle='modal' data-target='#showOrderModal'>" + data.order_id + " </a></td > \n\
                <td> <a data-order-id='" + data.order_id + "' data-toggle='modal' data-target='#showOrderModal'> " + data.customer_name + "</a></td > \n\
                <td> <a data-order-id='" + data.order_id + "' data-toggle='modal'data-target='#editOrderModal'> Edit </a></td > \n\
                <td> <a data-order-id='" + data.order_id + "' class='delete-link'> Delete </a>  </td> \n\
                </tr>";
