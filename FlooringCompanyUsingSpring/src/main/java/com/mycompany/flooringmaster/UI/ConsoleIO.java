...
/**
 * @author Kyle Sickels & Chris Glisch
 */
...
//BEGIN: getMultiChoiceInt
    ...
    public int getMultiChoiceInt(String question, String choices[], boolean addExitToChoices, String finalChoice, boolean trueForIndexFalseForSelection) {
       ...
       //minVal = (a < b) ? a : b;
        //if (trueForIndexFalseForSelection ? add=0 : add =1);
        ...
        while (true) {
            try {
                ...
                for (int i = 0; i < finalChoices.length; i++) {
                    if (finalChoices[i].equalsIgnoreCase(userResponse)) {
                        //EDIT THISSSSSSSSSSSSSSSSSSSSSSSSSSSSS
                        return i+add;
                    }
                }
                ...
                try {
                    ...
                    if (shortAnswer >= 0) {
                        for (int i = 0; i < finalChoices.length; i++) {
                            if (finalChoices[i].equalsIgnoreCase(finalChoices[shortAnswer-1])) {
                                //EDIT THISSSSSSSSSSSSSSSSSSSSSSSSSSSSS
                                return i+add;
                            }
                        }
...
//BEGIN: getMultiChoiceString
    ...
    public String getMultiChoiceListString(String question, String choices[], boolean addExitToChoices, String finalChoice, int n) {
        ...
        while (true) {
            try {
                ...
                for (int i = 1; i < (finalChoices.length + 1); i++) {
                    if ((i - 1)%n==0)
                        println("");
                    print("\t\t" + i + ") " + finalChoices[i - 1]);
                }
                System.out.print("\nChoice: ");
                userResponse = keyboard.nextLine();
                //vaildate the answer is a choice
                for (int i = 0; i < finalChoices.length; i++) {
                    //Added  ||userResponse.equals("") so that it may return no change
                    if (finalChoices[i].equalsIgnoreCase(userResponse)||userResponse.equals("")) {
                        return userResponse;
                    }
                }
                //Backup incase shorthand integer was used instead of string response
                try {
                    ...
                    if (shortAnswer >= 0) {
                        for (int i = 0; i < finalChoices.length; i++) {
                            if (finalChoices[i].equalsIgnoreCase(finalChoices[shortAnswer-1])) {
                                return finalChoices[shortAnswer-1];
                            }
                        }
                    }
                } catch (Exception ex) {
    ...
    public String getMultiChoiceListString(String question, String choices[]) {
        return getMultiChoiceListString(question, choices, false, "", 0);
    }
...
//BEGIN: getMultiChoiceString
    //asks <question> with <choices[]> and returns the String of the valid choice selected
    public String getMultiChoiceString(String question, String choices[], boolean addExitToChoices, String finalChoice) {
